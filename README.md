...
- 课堂视频页：弹出 jAccount 二维码，扫码后一次性拉取所有可下载课程的节次与下载 URL；二次进入直接展示缓存树，课程行显示总节数和已选节数（不展示课程代码），按课程 → 节次正向勾选；展开每门课后可在其内部「按节次范围批量勾选」（如 `1-4, 7`）+「全选 / 全清」；顶部 [全部展开 / 全部收起]；勾选框带强调色高亮动画
